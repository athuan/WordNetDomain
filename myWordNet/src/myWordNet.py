...
def demo2():
    ...
    wn = WordNetCorpusReader(nltk.data.find('corpora/wordnet2.0'), None)
    print('done loading2.0')
    S = wn.synset
    L = wn.lemma
#     thing = S("thing.n.1")
#     print(thing.name())
#     print(thing.offset())
    ...
    # open file
    file_out = open("synset_offset.txt", "w")
    file_domain = open("synset_domain.txt", "w")
    file_in = open("wn-domains-3.2-20070223", "r")
    file_in_synset = open("danhsachmasynset.txt")
    
    # the hash with key: synset_offset, value: domain
    domain_list = {}
    ...
    print("domain length: "+str(len(domain_list)))
    
    # assign domain for synset
    for synset in file_in_synset:
        sysnet = str(synset.trim())
        syn = S(synset)
        
    
#     syns = list(wn.all_synsets())
#     print("synsets length: "+str(len(syns)))
#     offsets_list = {}
#     for s in syns:
#         string = ""
#         offset = s.offset()
#         key = str(addZero(offset)+"-"+s.pos())
# #         print("key: "+key)
# #         print(str(key)+"-"+str(s))
# #         string = str(key)+"-"+str(s.name())+"\n"
#         domain_string = str(key)+"\t"+str(s.name())+"\t"+domain_list[key]+"\n"
#         file_domain.write(domain_string)
